fn main() {
    ...
    let code = "2"; // 查找的裤子编号
    println!("小胖开始查找编号为 {}的裤子", code);
    for trouser in &xiaopang_box_trousers {
        println!("查看裤子{}是不是编号为{}", trouser, code);
        if trouser.starts_with(code) {
            println!("找到了编号为 {} 的裤子{}", code, trouser);
        }
    }
}
