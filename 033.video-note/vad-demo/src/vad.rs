    ...
    F: Frame,
{
    ...
    let negated_threshold = F::Sample::EQUILIBRIUM.to_signed_sample() - threshold;
...
fn next_state<F>(state: State, frame: F, open_threshold: F::Sample, release_time: usize) -> State
where
    F: Frame,
{
    match state {
