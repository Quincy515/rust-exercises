...
// 因为 Rust trait 的孤儿规则，如果要想对已有的类型实现已有的 trait，需要简单包装一下
...
impl<'a> TryFrom<&'a Statement> for Sql<'a> {
    ...
    fn try_from(sql: &'a Statement) -> Result<Self, Self::Error> {
        match sql {
            ...
            _ => Err(anyhow!("We only support Select Query at the")),
        }
    }
}
...
impl<'a> TryFrom<Source<'a>> for &'a str {
    ...
    fn try_from(source: Source<'a>) -> Result<Self, Self::Error> {
        if source.0.len() != 1 {
            return Err(anyhow!("We only support signle data source at the moment"));
        }
